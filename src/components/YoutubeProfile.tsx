...
export default function YoutubeProfile({ 
  ...
  const channelsList = channelSettings.channels && channelSettings.channels.length > 0
  ...
  const currentIndex = activeChannelIndex < channelsList.length ? activeChannelIndex : 0;
  const activeChannel = channelsList[currentIndex];
  ...
  // Reset subscription status on active channel switch
  useEffect(() => {
    setIsSubbed(false);
    setSimulatedSubs(0);
  ...
  useEffect(() => {
    async function fetchProfile() {
      try {
        ...
        // Seamlessly use preloaded creator profile details
        setProfile({
          title: activeChannel.name || "TITAN GAMING 1M",
          description: `Official creator space for ${activeChannel.name || "TITAN GAMING 1M"}. Home to extreme gaming guides, pro-tier competitive battles, weekly interactive streams, and live companion rewards. Fasten your seatbelts and subscribe!`,
          customUrl: `@${(activeChannel.name || "titangaming1m").toLowerCase().replace(/[^a-z0-9]/g, "")}`,
          avatarUrl: "",
          bannerUrl: "",
          subscriberCount: activeChannel.channelId === "UCUIIdsKfR-Gn5_2rKzfzznQ" ? "1140000" : "248000",
          viewCount: activeChannel.channelId === "UCUIIdsKfR-Gn5_2rKzfzznQ" ? "48201950" : "5890000",
          videoCount: activeChannel.channelId === "UCUIIdsKfR-Gn5_2rKzfzznQ" ? "412" : "118"
        });
      } finally {
        setLoading(false);
      }
    ...
    fetchProfile();
  }, [activeChannel.channelId, activeChannel.apiKey, activeChannel.name]);
  ...
  const renderChannelSelector = () => {
    if (channelsList.length <= 1) return null;
    return (
      <div className="bg-cosmic-950/40 p-4 rounded-2xl border border-cosmic-800/80 mb-6 flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div className="space-y-1">
          <div className="flex items-center space-x-2 text-xs font-mono text-cyber-blue font-bold">
            <span className="h-2 w-2 rounded-full bg-cyber-blue animate-pulse" />
            <span className="uppercase tracking-widest">MULTI-CHANNEL MODE ACTIVE</span>
          </div>
          <p className="text-xs text-gray-400">
            Switch between the connected creator feeds to view active content, statistics, and milestone rewards.
          </p>
        </div>
          ...
          {channelsList.map((ch, idx) => {
            ...
            return (
              <button
                ...
                className={`px-4 py-2 rounded-xl text-xs font-bold transition-all duration-300 flex items-center space-x-1.5 cursor-pointer border ${
                  isSelected
                    ? "bg-gradient-to-r from-cyber-purple to-cyber-pink text-white border-transparent shadow-lg scale-105"
                    : "bg-cosmic-900/60 text-gray-400 border-cosmic-800 hover:text-white hover:bg-cosmic-800/60"
                }`}
              >
          ...
          <div className="relative overflow-hidden rounded-3xl border border-white/5 bg-[#0e0c1b]/40 backdrop-blur-md shadow-2xl transition-all duration-500">
            ...
            <div className="px-6 pb-6 pt-0 relative">
                ...
                <div className="flex flex-col sm:flex-row items-center sm:items-end space-y-4 sm:space-y-0 sm:space-x-5 text-center sm:text-left">
                  ...
                  <div className="space-y-1 sm:mb-1.5">
                    ...
                    <div className="flex flex-wrap items-center justify-center sm:justify-start gap-2 text-xs font-mono text-gray-400">
                      <span className="text-cyber-blue font-bold">{profile.customUrl || `@${profile.title.toLowerCase().replace(/[^a-z0-9]/g, "")}`}</span>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
                ...
                <div className="glass-card-premium rounded-2xl p-4.5 flex flex-col justify-between space-y-3">
                  <div className="flex items-center space-x-4">
                    ...
                    <div>
                      ...
                      <div className="text-xl font-extrabold text-white tracking-tight mt-1">
                        {currentTotalSubs.toLocaleString()}
                      </div>
                    </div>
                  </div>
                  <div className="border-t border-white/5 pt-2 flex items-center justify-between text-[10px] font-mono">
                    <span className="text-gray-400">Original count added</span>
                    <span className="text-cyber-pink font-bold">
                      {baseSubs.toLocaleString()}
                    </span>
                  </div>
                </div>
              ...
              <div className="glass-card-premium rounded-2xl p-6 space-y-4">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                  <div className="space-y-1">
                    <div className="flex items-center space-x-1.5 text-xs font-mono text-[#a5a1ff]">
                      ...
                      <span className="uppercase tracking-widest font-bold">Community Milestones</span>
                    </div>
                ...
                <div className="space-y-1.5">
                  ...
                  <div className="flex justify-between text-[10px] font-mono text-gray-500 uppercase tracking-widest font-semibold">
                    ...
                    <span>Original: {baseSubs.toLocaleString()}</span>
                  </div>
                </div>
              </div>
